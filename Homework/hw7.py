...
# Favorite flavor of dorito : I dont eat doritos but I like BBQ lays
# *********************************************
...
class Term:
    ...
    def __call__(self, input): # myTerm(input)
        ...
        exponent_eval = input**self.exp
        result = self.coef*exponent_eval
        return result
    ...
    def __neg__(self): # otherTerm = -myTerm
        ...
        neg_coef = self.coef*-1
        return Term(neg_coef, self.exp)
...
# exponent is strictly decreasing
    ...
    def __neg__(self): # negPoly = -myPoly
        ...
        neg_list = self.polyList.copy()

        for x in neg_list:
            x.coef = x.coef*(-1)
            
            
        return LinkedPolynomial(neg_list)
    ...
    def __mul__(self, otherPoly): # mulPoly = myPoly * otherPoly
        ...
        for x in self.polyList:
            ...
            for y in other.polyList:
    
                result_exp = x.exp + y.exp
                result_coef = x.coef * y.coef

                combined_T = Term(result_coef, result_exp)

                if result_exp in term_dict:
                    term_dict[result_exp] += combined_T
                    
                else:
                    term_dict = combined_T

                for z in term_dict:
                    term_list.append(term_dict[z])
